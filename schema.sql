...
CREATE DOMAIN USER_ROLE_TYPE AS VARCHAR(20)
    CHECK (VALUE IN ('ADMIN', 'INSTRUCTOR', 'STUDENT'));
...
CREATE DOMAIN TERM_NAME_TYPE AS VARCHAR(20)
    CHECK (VALUE ~ '^(Winter|Spring|Summer|Fall) [0-9]{4}$');
...
CREATE TABLE SECTION (
    ...
    TERM_CODE INT NOT NULL REFERENCES TERM(TERM_CODE),
...
CREATE TABLE INSTRUCTOR (
    USER_ID INT PRIMARY KEY REFERENCES USER_(USER_ID) ON DELETE CASCADE,
...
CREATE TABLE LAB_PROGRESS_LOG (
    ...
    PROG_ID INT NOT NULL REFERENCES LAB_PROGRESS(PROG_ID) ON DELETE CASCADE,
